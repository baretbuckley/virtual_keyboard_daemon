...
int initKeyBoard(struct KeyBoard *keyboard) {
    ...
    if (keyboard->fd < 0) {
        fprintf(stderr, "Failed to open '/dev/uinput/ in creating keyboard. Linux Error Code %i\n", errno);
        return -1;
    }

    if (ioctl(keyboard->fd, UI_SET_EVBIT, EV_KEY)) {
        fprintf(stderr, "ioctl UI_SET_EVBIT EV_KEY failed. Linux Error Code %i\n", errno);
    }
    ...
    for (unsigned int i=0; i < sizeof(key_list)/sizeof(int); i++) {
        if (ioctl(keyboard->fd, UI_SET_KEYBIT, key_list[i])) {
    ...
    struct uinput_setup usetup = {
        ...
        .name = "vkey virtual device",
        .ff_effects_max = 0
    };

    if (ioctl(keyboard->fd, UI_DEV_SETUP, &usetup)) {
        fprintf(stderr, "UI_DEV_SETUP failed\n");
        return -1;
    }
    if (ioctl(keyboard->fd, UI_DEV_CREATE)) {
